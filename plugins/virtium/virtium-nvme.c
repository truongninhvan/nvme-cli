...
static void vt_init_attr_name_map()
{
	...
	vt_insert_attr_name_map(table,	attr_index,	true,		0x09,	"Power_on_hour_count",	"");	attr_index++;
	vt_insert_attr_name_map(table,	attr_index,	true,		0x0c,	"Power_cycle_count",	"");	attr_index++;
	...
	vt_insert_attr_name_map(table,	attr_index,	true,		0xa8,	"Max_PE_cycle",	"");	attr_index++;
	...
	vt_insert_attr_name_map(table,	attr_index,	true,		0xc0,	"Power-off_retract_count",	"");	attr_index++;
	vt_insert_attr_name_map(table,	attr_index,	true,		0xc2,	"Enclosure_temperature",	"");	attr_index++;
...
static void vt_header_to_string(const struct vtview_log_header *header, char *text)
{
	...
	snprintf(tempbuff, MAX_HEADER_BUFF,
		"\"devices\":[{\"model\":\"%s\",\"port\":\"%s\",\"SN\":\"%s\",\"type\":\"%s\",\"identify\":\"%s\",\"firmwareSlot\":\"%s\"}]}\n",
		header->raw_ctrl.mn, header->path, header->raw_ctrl.sn, header->device_type, identext, fwtext);
	strcat(text, tempbuff);
}
...
static int vt_get_device_type(int fd, struct vtview_log_header *header)
{
	int err = -1;
  __u8	log_data[512];
  
  strcpy(header->device_type, "NVMe");
  if (vt_is_vendor_support(header->raw_ctrl.vid)) {
    if (VIRTIUM_VENDOR == header->raw_ctrl.vid) {
      memset(log_data, 0, 512);
      err = nvme_get_log(fd, NVME_NSID_ALL, REALTEK_NVME_LOG_VENDOR_SPEC, false, sizeof(log_data), log_data);
      if (!err) {
        strcpy(header->device_type, "NVMe_RT");
      }
    }
  }
